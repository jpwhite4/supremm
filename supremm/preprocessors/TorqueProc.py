...
class TorqueProc(PreProcessor):
    """ Parse and analyse the proc information for a job that ran under Torque
        where linux cgroups were enabled.
    """
    ...
    def __init__(self, job):
        super(TorqueProc, self).__init__(job)
    ...
    @staticmethod
    def cgroupparser(s):
        """ Parse linux cgroup string and  the extract job id
        """

        groups = s.split(";")
        groupre = re.compile(r"^cpuset:/torque/(\d+(?:\[\d+\])?)(?:\.[^\.].*)?")
